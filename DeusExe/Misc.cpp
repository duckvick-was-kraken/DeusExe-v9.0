...
namespace
{
    //Try and walk back the crash for a trace
    ...
    void ReportFatal(const wchar_t* const pszReason, const EXCEPTION_POINTERS* const pExceptionInfo)
    {
        ...
        Misc::GetCrashContext(szContext, _countof(szContext));
        ...
        Misc::LogCrashDetail(pExceptionInfo);
...
const wchar_t* Misc::FormatScriptContext(wchar_t* const pszBuffer, const size_t count, const UObject* const pObject, const UStruct* const pFunction)
{
    wchar_t szObject[1024];
...
void Misc::LogCrashDetail(const EXCEPTION_POINTERS* const pExceptionInfo)
{
...
void Misc::SetScriptTrace(const UObject* const pObject, const UStruct* const pFunction)
{
...
void Misc::SetCrashPhase(const wchar_t* const pszPhase)
{
...
const wchar_t* Misc::GetCrashContext(wchar_t* const pszBuffer, const size_t count)
{
    ...
    {
        wcsncpy_s(szScript, g_bVerboseLogging ? L"<no hooked-native context recorded>" : L"<verbose logging disabled>", _TRUNCATE);
    }
